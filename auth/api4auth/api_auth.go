...
type AuthHandlerWithUser func(ctx context.Context, w http.ResponseWriter, r *http.Request, authInfo token4auth.AuthInfo, user dbo4userus2.UserEntry)
...
func AuthOnlyWithUser(handler AuthHandlerWithUser) strongoapp.HttpHandlerWithContext {
	return AuthOnly(func(ctx context.Context, w http.ResponseWriter, r *http.Request, authInfo token4auth.AuthInfo) {
		...
		if userID = common4all2.GetUserID(ctx, w, r, authInfo); userID == "" {
			logus.Warningf(ctx, "userID is empty")
			return
		}

		//user, err := dal4userus.GetUserByID(ctx, nil, userID)
		var user dbo4userus2.UserEntry
		var err error

		if common4all2.HasError(ctx, w, err, dbo4userus2.UsersCollection, userID, http.StatusInternalServerError) {
			return
		}
