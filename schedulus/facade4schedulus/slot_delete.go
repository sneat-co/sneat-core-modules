...
func DeleteSlots(ctx context.Context, user facade.User, request dto4schedulus.DeleteHappeningSlotRequest) (err error) {
...
func removeSlotFromSingleHappening(
	...
	request dto4schedulus.DeleteHappeningSlotRequest,
) error {
...
func removeSlotFromHappeningDto(
	...
	if request.Weekday == "" {
		...
		if len(slots) < len(happening.Dto.Slots) {
			if len(slots) == 0 {
				happening.Dto.Status = models4schedulus.HappeningStatusDeleted
...
func removeSlots(slots []*models4schedulus.HappeningSlot, slotIDs []string) []*models4schedulus.HappeningSlot {
	result := make([]*models4schedulus.HappeningSlot, 0, len(slots))
...
func removeWeekday(slot *models4schedulus.HappeningSlot, weekday models4schedulus.WeekdayCode) (changed bool) {
	weekdays := make([]models4schedulus.WeekdayCode, 0, len(slot.Weekdays))
